...
export default class SimpleAvatar {
  ...
  private TAG = "[SIMPLE MINIPROGRAM AVATAR]";
  private status: AvatarStatus = AvatarStatus.close;
  private sessionId: string = '';
  
  // 组件实例
  private networkMonitor: NetworkMonitor;
  private webSocket: MiniProgramWebSocket | null = null;
  ...
  private resourceLoadQueue: string[] = [];
  ...
  public onStatusChange: (status: AvatarStatus) => void = () => {};
  public onMessage: (error: any) => void = () => {};
  ...
  private setupEventHandlers(): void {
    ...
    // 资源加载事件
    this.on(EventType.RESOURCE_LOADED, (resource: string) => {
    ...
    // 动画事件
    this.on(EventType.ANIMATION_START, (animationName: string) => {
      if (this.options.onAnimationStart) {
        this.options.onAnimationStart(animationName);
      }
    });

    this.on(EventType.ANIMATION_END, (animationName: string) => {
   ...
   * 处理网络状态变化
   */
  private handleNetworkChange(isOnline: boolean, message: string) {
    if (isOnline) {
  ...
  private async connectWebSocket(): Promise<void> {
    try {
      ...
      const sdkError = {
        code: EErrorCode.WEBSOCKET_CONNECT_ERROR,
        message: `WebSocket 连接失败: ${(error as Error).message}`,
        timestamp: Date.now()
      };
      this.onMessage(sdkError);
      throw error;
   ...
   * 处理渲染数据
   */
  private handleRenderData(payload: any): void {
    // 这里可以处理来自服务器的渲染数据
   ...
   * 处理语音状态
   */
  private handleVoiceStatus(payload: any): void {
  ...
  async init(params: IInitParams): Promise<void> {
    ...
    try {
      ...
      }) as WebGLRenderingContext | null; // 类型断言
   ...
   * 预加载资源
   */
  private async preloadResources(resources: string[], onProgress?: (progress: number) => void): Promise<void> {
   ...
   * 内部加载单个资源
   */
  private async internalLoadResource(url: string, type: string): Promise<any> {
   ...
   * 根据URL判断资源类型
   */
  private getResourceType(url: string): 'model' | 'texture' | 'audio' | 'animation' | 'other' {
   ...
   * 让数字人说话
   */
  speak(text: string, isStart: boolean = true, isEnd: boolean = true, extra: any = {}): void {
   ...
   * 加载额外资源
   */
  async loadResource(url: string, type?: 'model' | 'texture' | 'audio' | 'animation' | 'other'): Promise<boolean> {

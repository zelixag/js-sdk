...
import { IAvatarOptions, IInitParams, AvatarStatus, EErrorCode } from './types';
...
export default class MiniProgramAvatar {
  ...
  private TAG = "[MINIPROGRAM AVATAR]";
  private status: AvatarStatus = -1 as AvatarStatus; // -1 表示未初始化
  ...
  private connectToServer(): void {
    ...
    this.webSocket.on('disconnect', (res: any) => {
   ...
   * 渲染帧
   */
  private renderFrame(payload: any): void {
    if (!this.gl) {
      return;
    }

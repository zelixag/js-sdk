...
export class DebugOverlay {
    ...
    update() {
        ...
        if (window.performanceTracker) {
            const rawMetrics = window.performanceTracker.getVideoMetrics();
        ...
        const networkInfo = navigator.connection
            ? `网络类型: ${navigator.connection.effectiveType}；下行网速: ${navigator.connection.downlink}Mbps；往返延迟: ${navigator.connection.rtt}ms`
            : "N/A";
      ...
      <div style="border-top: 1px solid #444; padding-top: 8px;">
        ...
        <pre style="margin: 0; padding: 6px; background: rgba(255,255,255,0.05); border-radius: 4px; font-size: 10px; line-height: 1.3; color: ${this.errors.length > 0 ? "#ff9999" : "#888"}; max-height: 120px; overflow-y: auto;">${this.formatErrors(false)}</pre>
        <pre style="margin: 0; padding: 6px; background: rgba(255,255,255,0.05); border-radius: 4px; font-size: 10px; line-height: 1.3; color: ${this.errors.length > 0 ? "#ff9999" : "#888"}; max-height: 120px; overflow-y: auto;">${this.formatErrors(true)}</pre>
      </div>
      <div style="margin-bottom: 8px;">
    ...
    formatPerfMetrics() {
        ...
        return Object.entries(this.perfMetrics)
            .map(([key, value]) => `${key}:  最近一次: ${value.last.toFixed(2)}ms;  平均: ${value.avg.toFixed(2)}ms;  次数: ${value.count}`)
            .join("\n");
    }
    trackFPS() {
        const loop = (now) => {

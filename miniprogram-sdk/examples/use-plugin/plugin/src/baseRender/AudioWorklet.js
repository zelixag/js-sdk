...
export default class PCMAudioPlayer {
    constructor() {
        ...
        this.FRAME_ALIGNMENT = 2000; // 帧对齐字节数
        this.WORKLET_SCRIPT_URL = workletUrl; // 改为本地引入，内网客户无法使用远程资源
        // readonly WORKLET_SCRIPT_URL: string = 'http://localhost:5173/pcm-audio-processor.js'; // Worklet脚本路径（需与HTML同域）
        // 实例状态与资源
        this.audioCtx = null;
        this.audioWorkletNode = null; // Worklet节点（主线程与Worklet通信桥梁）
        this.isInitialized = false; // 初始化状态（Worklet加载完成）
            ...
            this.audioWorkletNode.port.onmessage = (e) => {
                switch (e.data.type) {
                    ...
                    case "syncVolume":
                        ...
                        if (this.audioWorkletNode?.parameters?.get("volume")?.value !==
                            undefined) {

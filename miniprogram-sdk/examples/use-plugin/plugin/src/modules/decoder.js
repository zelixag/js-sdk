...
export default class ParallelDecoder {
    constructor(options) {
        ...
        this.cacheVideoCount = 5; // 缓存视频数量
        this.maxVideoCount = 2;
        this.hardwareAcceleration = "default";
        this.MAX_LOAD_VIDEO_TIMEOUT_MS = 2000; // 最大加载视频超时时间（毫秒）
    ...
    async _tryStartNext() {
        ...
        if (this.currentParallel < this.maxParallel && this.queue.length > 0) {
            ...
            window.performanceTracker.markStart(performanceConstant.load_video, bodyInfo);
            if (!bodyInfo.n) {
                this.currentParallel--;
                this._tryStartNext();
                this.reportMessage({
            ...
            window.performanceTracker.markEnd(performanceConstant.load_video, bodyInfo);
            window.performanceTracker.markStart(performanceConstant.decode_video, bodyInfo);
            this._startWorker({
                name: bodyInfo.n,
                id: bodyInfo.id,
    ...
    _startWorker(file) {
        ...
        const onMessage = (e) => {
            ...
            if (e.data.type === "frame") {
                ...
                window.performanceTracker.markEnd(performanceConstant.decode_video, file);
                const bodyFrameCount = this.bodyFrameCountMap.get(file.name) ?? 0;

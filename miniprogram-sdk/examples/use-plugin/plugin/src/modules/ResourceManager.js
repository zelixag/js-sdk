...
import { IBRAnimationGeneratorCharInfo_NN, } from "../utils/DataInterface";
...
export default class ResourceManager {
    constructor(options) {
        this.TAG = "[ResourceManager]";
        this.offlineIdle = [];
        this.offlineCache = new Map();
        this.networkInfos = [];
        this.bg = null;
        this.progress = 0; // start session 以后之后资源下载进度
        // 视频缓存相关
        this.videoCache = new Map();
        this.maxCacheSize = 10 * 1024 * 1024; // 5MB 默认缓存大小
        this.maxCacheEntries = 20; // 最大缓存条目数
        this.cacheCleanupInterval = null;
        this.isProcessingVideo = false; // 防止在视频处理过程中清理缓存
        // 新增：正在下载的视频跟踪
    ...
    async loadMouthShapeLib(onDownloadProgress) {
        try {
            ...
            try {
                ...
                try {
                    // pako 解压
                    const decompressed = Pako.ungzip(new Uint8Array(gz_data));
            ...
            const char_info = new IBRAnimationGeneratorCharInfo_NN(char_data, {
